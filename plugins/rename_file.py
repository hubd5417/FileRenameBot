...
async def rename_doc(bot, update):
    ...
    if update_channel:
        try:
            ...
            if user.status == "kicked":
                await update.reply_text("Sorry, You've Been Banned From Using Me!")
                return
        except UserNotParticipant:
            await update.reply_text(
                text="**You must join the updates channel to use this bot.**",
                reply_markup=InlineKeyboardMarkup([
    ...
    if (" " in update.text) and (update.reply_to_message is not None):
        ...
        caption_data = await get_caption(update.from_user.id)

        try:
            caption_text = caption_data.caption.format(filename=file_name)
        except:
            caption_text = f"<code>{file_name}</code>"
        ...
        # 🔥 VIDEO OR DOCUMENT CHECK
        if file_name.lower().endswith((".mp4", ".mkv", ".mov", ".webm")):

            await bot.send_video(
                chat_id=update.chat.id,
                video=new_file_name,   # ✅ FIXED
